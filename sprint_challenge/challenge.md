...
- When I look at the functions that have docstring, I know how the function work, 
what is its condition to return the value it suppose to return thus I can double check the code 
to see if it stays in line with the docstring. I think other people will do the same things in company.
- Each containers has an underlying operating system with required packages pre-installed 
so that the code/app always run fine in those environment regardless of ones' computer operating system 
